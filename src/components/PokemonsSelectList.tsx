...
export const PokemonsSelectList: React.FC<IPokemonsSelectList> = ({
  userPokemons,
  selectedPokemon,
  setSelectedPokemon
}: IPokemonsSelectList) => {
  ...
  useEffect(() => {
    if (data?.pokemons) {
      ...
      const newPokemonsList = data.pokemons.filter(
        pokemon => !idsOfUserPokemons.includes(pokemon.id)
      )
