...
export const isUserAuthorized = (
  ...
  return (
    ...
    authenticatedUser._id.toString() === targetUser._id.toString()
  );
};
...
export const abstractUserBasedOnAuthorizationLevel = (
  req: IRequest,
  authenticatedUser: IUser,
  response: IUserDocument,
  allPokemons?: IPokemonDocument[],
  shouldPopulatePokemons: boolean = true
): IUser => {
  const user: IUser = formatUserDocument(
    req,
    response,
    allPokemons,
    shouldPopulatePokemons
  );
  const { _ID, ROLE, EMAIL, CREATED_AT, UPDATED_AT } = USER_KEYS;
  if (
    authenticatedUser.role !== USER_ROLES.ADMIN &&
    authenticatedUser._id.toString() !== response._id.toString()
  ) {
...
export const formatUserDocument = (
  req: IRequest,
  response: IUserDocument,
  allPokemons?: IPokemonDocument[],
  shouldPopulatePokemons: boolean = true
