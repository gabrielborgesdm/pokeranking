...
export const getAllUsers = async (req: IRequest, res: NextApiResponse) => {
  ...
  if (response) {
    ...
    users = response.map((userResponse) =>
      abstractUserBasedOnAuthorizationLevel(
        req,
        req.user,
        userResponse,
...
export const getUserByUsername = async (
  ...
  const allPokemons = await pokemonRepository.getAll();
  const user: IUser = abstractUserBasedOnAuthorizationLevel(
    req,
    req.user,
    response,
    allPokemons
...
const isOkayToExecuteMutation = (
  authenticatedUser: IUser,
  response: IUserDocument
): IResponse => {
  if (!response) {
...
export const updateUser = async (req: IRequest, res: NextApiResponse) => {
  ...
  const allPokemons = await pokemonRepository.getAll();
  const user = abstractUserBasedOnAuthorizationLevel(
    req,
    req.user,
    updateResponse,
    allPokemons
...
export const storeUser = async (req: IRequest, res: NextApiResponse) => {
  ...
  if (await getUser({ $or: [{ username }, { email }] }))
    return sendResponse(req, res, USER_ALREADY_REGISTERED);
  userInfo.password = await hashPassword(userInfo.password);
...
export const login = async (req: IRequest, res: NextApiResponse) => {
  ...
  const allPokemons = await pokemonRepository.getAll();
  sendResponse(req, res, SUCCESS, {
    token,
    user: formatUserDocument(req, response, allPokemons),
  });
};
